...
const CATS = [
  { key: 'films', label: 'Films', icon: '[F]' },
  { key: 'series', label: 'Séries', icon: '[S]' },
  { key: 'animes', label: 'Animés', icon: '[A]' },
];
...
export default function Home({ navigation }) {
  ...
  const clearSearch = () => { setQ(''); setSr(null); if (debounceRef.current) clearTimeout(debounceRef.current); };
  const formatTime = (s) => { const m = Math.floor(s / 60); const sec = Math.floor(s % 60); return `${m}:${String(sec).padStart(2, '0')}`; };

  if (loading) return <View style={s.c}><View style={s.ctr}><Image source={require('../../assets/icon.png')} style={s.logo} /><ActivityIndicator size="large" color="#e94560" /><Text style={s.lt}>Connexion...</Text></View></View>;
  ...
  const ListHeader = (
    <View>
        ...
        <View>
          <View style={s.hrRow}>
            ...
            <TouchableOpacity onPress={refreshHistory}>
              <Text style={{ color: '#8892b0', fontSize: 14 }}>↻</Text>
            </TouchableOpacity>
          </View>
      ...
      <View style={s.cr}>{CATS.map(c => <TouchableOpacity key={c.key} style={s.cb} onPress={() => navigation.navigate('Category', { category: c.key, label: c.label, url })}><Text style={{ color: '#e94560', fontSize: 16 }}>{c.icon}</Text><Text style={s.cl}>{c.label}</Text></TouchableOpacity>)}</View>
  ...
  return (
    <View style={s.c}>
      <View style={s.h}>
        ...
        <View style={s.hr}>
          {!url ? <TouchableOpacity onPress={rediscover} style={s.rc}><Text style={{ color: '#fff', fontSize: 16 }}>↻</Text></TouchableOpacity> : null}
          <TouchableOpacity onPress={() => navigation.navigate('Settings', { url, onRefresh: rediscover })}><Text style={{ color: '#fff', fontSize: 20 }}>⚙</Text></TouchableOpacity>
        </View>
      </View>

      <View style={s.sr}>
        <View style={s.sb}>
          <Text style={{ color: '#8892b0', fontSize: 16 }}>🔍</Text>
          ...
          {q ? <TouchableOpacity onPress={clearSearch}><Text style={{ color: '#8892b0', fontSize: 16 }}>✕</Text></TouchableOpacity> : null}
        </View>
        <TouchableOpacity style={s.sbtn} onPress={() => doSearch(q)}><Text style={{ color: '#fff', fontSize: 16 }}>→</Text></TouchableOpacity>
      </View>
